...
- **Streak persistence layer** — each card grade now records the current date in `localStorage` (key `poke-memory:streak:v1`) via a new `lib/streak` module. Provides `recordReview`, `computeStreak`, `loadStreakData`, and `saveStreakData` helpers ready for the streak UI (issue [#6](https://github.com/fraserbrookhouse/poke-memory/issues/6)). Closes [#41](https://github.com/fraserbrookhouse/poke-memory/issues/41).
