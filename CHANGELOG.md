...
- **CI now runs on `/fix`-cycle commits** — pushes made by `claude-code-action` during a `/fix` cycle previously went out under the runner's `GITHUB_TOKEN`, causing GitHub to suppress the `pull_request: synchronize` event and leaving the PR with no `test` check on the new head SHA. Setting the App-installation token as a global git credential before the action step means all pushes now authenticate as `poke-memory-bot`, so `ci.yml` fires correctly on every head SHA. Applies to `auto-pr.yml` (fix cycles) and both `auto-issue.yml` jobs (implement and continue). Closes [#121](https://github.com/fraserbrookhouse/poke-memory/issues/121).
