...
- PWA badge and document-title badge now update reactively after grading a card on mobile Safari, and share queue-count computation with the Stats dashboard.
- Forecast bars on the Stats dashboard now count all eligible card types (name, evolution, reverse, cry), matching the today bar.
