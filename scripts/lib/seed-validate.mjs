 ...
 *   - generated-flavor.json has all `id` values from generated.json.
 *   - generated-locale-names.json has all `speciesId` values for default-form records.
 *
 * Returns an array of ValidationResults (one per shard) so callers can report
 * them individually.
 *
 ...
 */
export function validateShards(generated, core, flavor, chains, localeNames) {
  const results = [];
  ...
  const missingFlavor = allIds.filter((id) => !flavorIdSet.has(id));
  results.push(
    missingFlavor.length === 0
      ? {
          ok: true,
          message: `generated-flavor.json OK: all ${allIds.length} records present.`,
        }
      : {
          ok: false,
          message:
            `FAIL: generated-flavor.json is missing ${missingFlavor.length} record(s).\n` +
