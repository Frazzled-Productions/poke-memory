...
**Met (record only).** This document, together with the privacy notice and `docs/cookies-pecr.md`, constitutes the substance of a DPIA-style risk assessment for the children's-data angle. The processing is low-risk: minimal data, no profiling, no advertising, RLS-isolated per-user storage, and a guest path that processes nothing server-side. A standalone formal DPIA is recorded as a follow-up (see below) for completeness, but no high-risk processing has been identified that would make one legally mandatory.
