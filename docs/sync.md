...
7. **Online-reconnect catch-up** (`useOnlineReconnectSync`, mounted via `OnlineReconnectSync` in the root layout) — fires on the browser `online` event. Performs a full pull-then-push catch-up: (1) `pullAndMerge` brings local state up to date; (2) if `lastPushFailed` is true, re-pushes the card subset selected by the same heuristic as path 5. Unlike path 5, this hook pulls first because it runs automatically on reconnect rather than on explicit user action — the pull guards against the #293 failure mode before any push. See [Offline behaviour and online-reconnect catch-up](#offline-behaviour-and-online-reconnect-catch-up) for the full detail. This hook also listens for `BACKGROUND_SYNC_REPLAY` messages from the service worker (path 8 below) and runs the same pull-then-push logic on receipt — this prevents concurrent pushes from both the SW and the running app.

8. **Background Sync replay** (`lib/sync/backgroundSync.ts`, SW handler in `app/sw.ts`, #1054) — progressive enhancement for Chromium on Android. When a push via `useSyncOnUnload` fails (sendBeacon rejected or fetch returned non-2xx), `registerBackgroundSync()` is called to register the `poke-memory:grade-sync` sync tag. The browser fires a `sync` event in the service worker once connectivity returns, even if every app tab has been closed. The SW handler then: (a) checks for active window clients — if any are present it posts `BACKGROUND_SYNC_REPLAY` and returns, delegating to the running `useOnlineReconnectSync` hook; (b) when no clients are active, reads the persisted pending-grade queue from IndexedDB (mirrored there by `savePendingQueue` alongside the localStorage write) and POSTs it to `/api/sync` with `credentials: "include"`. On a non-2xx response the `waitUntil` promise rejects so the browser will retry the sync event. On unsupported browsers (`'SyncManager' in window` is false — Firefox, Safari), `registerBackgroundSync()` is a no-op and path 7 remains the sole offline catch-up mechanism.
