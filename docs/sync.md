...
- `OLD.seen_in_pasture = true AND NEW.seen_in_pasture = false` — `seen_in_pasture` flipping from `true` to `false` (added in migration 017; the flag is a one-way "user has scouted this card in the Higher-or-Lower pasture minigame" marker, so any client write that clears it is a sync bug).
...
- **`pushSession` is not deleted** — it remains the batched escape hatch that future orchestrators (or recovery paths) can call. Today nothing in `app/` or `components/` invokes it; the per-grade and unload paths use `pushSingleCard` instead.
