...
4. **Manual sync (`useManualSync`)** — wired to the Stats-page "Sync" button. **Pulls before pushing** — see [Invariants](#invariants-and-destructive-write-protection-read-first) above for why. Order: load local → pull cards → merge → save → push merged cards → streak (pull → union-merge → save → push) → settings (push local; pull only when `hasStoredSettings()` is false) → regional-prefs (`pushRegionalPrefs` + `pullRegionalPrefs` as a best-effort auxiliary leg). Cards drive success/error state; streak, settings, and regional-prefs failures `console.warn` and continue. `pushSession` (batched) is reused for the cards push.
