...
test.describe("Sign-in sheet (#1669)", () => {

  test("opens SignInSheet dialog with GitHub and Google options and value-prop heading", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("SignInSheet closes when Escape is pressed", async ({ page, browserName }) => {
    test.skip(browserName !== "chromium", "keyboard check only needs one engine");
    await page.goto("/");
  ...
  test("SignInSheet stays within viewport on mobile (#441)", async ({
    page,
    browserName,
  }) => {
    test.skip(
...
test.describe("Evolution edge card prompt (#262)", () => {
  test("renders 'What does {preEvo} evolve into {trigger}?' for an edge card", async ({ page }) => {
    ...
    // Pre-dismiss the modal after the settings override so the flag is merged
    // into the final settings value rather than being clobbered.
    await addOnboardingPreDismiss(page);
