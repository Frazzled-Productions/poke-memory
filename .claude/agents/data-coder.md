...
13. **Milestone WIP pushes for a big single-file epic (2026-06-07 retro).** If the brief has you implementing a multi-sub-issue epic in one PR (e.g. a large single-file restructure, or any epic whose sub-issues all touch the same file), `git push origin <your-branch>` a WIP commit after EACH sub-issue lands rather than working in one long unpushed silence. The orchestrator watches your branch tip for liveness - a tip that moves every sub-issue proves progress; a single push at the very end is indistinguishable from a stall for an hour.
