...
10. **Mandatory test-coverage rules (AGENTS.md "Mandatory coverage rules") — not optional.** (a) **State in AND out:** if your change introduces or gates on a state (a flag, mode, setting, signed-in vs guest, or **empty vs populated** data — including a derivation that returns a list that can be empty), unit-test BOTH sides, not just the populated happy path. (b) **Names/labels in every locale:** if a helper resolves, formats, or returns a Pokémon name or user-facing label, resolve names through the pure `lib/pokemon/localeNames.ts::getLocaleName` so every consuming surface — both hooks and non-hook render paths like the Pokédex grid — can render the correct locale value in all supported locales (`en`/`ja`/`zh-Hans`/`zh-Hant`), and unit-test the non-`en` cases. A locale-aware value reachable only via a React hook leaves perf-exempted surfaces (the grid) stuck on English — the #1327 gap.
