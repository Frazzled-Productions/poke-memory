...
3. **Before writing any code that adds or modifies persisted data — a new table, a new column on `card_reviews`, a new field in `user_settings.settings`, a new sync flow — read `docs/persistence.md` first** (AGENTS.md "Adding a feature that needs to persist data" points there), and confirm supabase-expert has reviewed the schema if it's non-trivial. The runbook covers the new-table checklist (RLS with SELECT + INSERT as the append-only baseline, opt-in UPDATE / DELETE only when justified, FK cascade), the JSONB-vs-new-table decision, the regression-trigger pattern, and the apply step (`mcp__supabase__apply_migration` **before merge**). The `migration-check.yml` workflow asserts file-vs-applied parity on every PR; if you forget to apply, CI complains.
