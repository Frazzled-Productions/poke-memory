...
9. **After a heading / section-rename sweep, run the affected specs LOCALLY until green - do not grep-and-hope (2026-06-07 retro).** When the change renames sections or changes heading levels across many surfaces, grep alone repeatedly misses stale `getByRole("heading", { level: N })` and renamed-section assertions (this hit ~4× in the 2026-06-07 batch). Reconciling the specs means RUNNING them against a local dev server until they pass, not patch-by-inspection: `npm run dev` in one terminal, then `PLAYWRIGHT_BASE_URL=http://localhost:3000 npx playwright test e2e/<affected>.spec.ts` under the pinned `nvm use` Node. Grep finds the obvious renames; the local run finds the level-N and accessible-name mismatches grep cannot see.
10. **Milestone WIP pushes for a big multi-spec epic (2026-06-07 retro).** If the brief has you reconciling specs across a large epic in one PR, `git push origin <your-branch>` a WIP commit after each spec area lands rather than working in one long unpushed silence - the orchestrator watches your branch tip for liveness.
11. **Run the build gate as SEPARATE commands, not the bundled `npm run pre-pr` (2026-06-07 retro).** Run `npm run lint`, `npm run typecheck`, `npm run build`, `npm test` each as its own command with a one-line status after it. A single long blocking command emits NOTHING to your transcript until it returns, which the orchestrator cannot distinguish from a stall.
12. **A service-worker-dependent spec MUST be validated against a local PRODUCTION build before pushing - structural-only validation is not enough (2026-06-08 retro).** The service worker only registers in a production build, so any spec that exercises offline behaviour, SW caching, precache, or `navigator.serviceWorker` cannot be validated against `npm run dev` - it will pass structurally and then fail on CI (the #1773 offline-grid spec shipped green-on-structure and went red on chromium: the SW was not controlling the page when `setOffline` was called, `page.reload` got `net::ERR_FAILED`). Build and serve a production build (`npm run build` then `npm run start`, confirm `/sw/sw.js` is served), then run the spec against it (`PLAYWRIGHT_BASE_URL=http://localhost:3000 npx playwright test e2e/<spec> --project=chromium`) under the pinned `nvm use` Node, and iterate until it passes 2-3× consecutively. SW-control gotcha to handle explicitly: a SW registered on first visit does not CONTROL the current page until a reload - warm + reload + wait for `navigator.serviceWorker.controller != null` BEFORE going offline. If you genuinely cannot serve a prod build locally, STOP and report the blocker rather than pushing an unvalidated SW spec.
