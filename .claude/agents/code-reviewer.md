...
1. Run `git status`, then review the **cumulative diff that will land on the base branch** - the whole PR as it squash-merges, never just the last commit. Use the three-dot range against the PR's actual base: `git diff origin/qa...HEAD` for batch/feature PRs (the usual case), or `git diff origin/main...HEAD` for a `hotfix` PR. If a PR exists you can equivalently use its diff endpoint. Do **not** review `git diff HEAD~1` or "the last commit" / "the fix commit": (a) multi-commit PRs squash into one commit on `qa`, so the cumulative diff is what actually lands and what a future reader sees; and (b) some bugs only appear across commits (an early commit removes a guard, a later one re-adds it elsewhere - net-correct but a per-commit review misses the journey). For the iteration story when it matters, also read the per-commit history (`git log origin/qa..HEAD --format='%h %s'`), but the primary review surface is always the cumulative range.
...
3. Check, in order:
   ...
   - **Fragmentation**: Any new direct field access on a domain object (e.g. `p.displayName`, `card.name`, `pokemon.sprite`), inline date formatting, ad-hoc mastery check, or inline Tailwind literal that duplicates a `lib/utils/class-names.ts` constant - that should route through a helper instead? If yes, raise as a **Blocker** tagged "fragmentation" with the existing helper name (or the proposed-helper name if none exists yet). The repeated partial-fix cycles around Pokémon-name locale (#1311 → #1318 → audit-fix) are the worked example this check exists to catch: with rendering split across many call sites, every cross-cutting change (locale, accessibility attribute, A/B test) needs a full audit and any miss ships as a bug. The canonical helper list and rationale live in AGENTS.md "Single source of truth for shared concepts".
   - **Discoverability**: When the change adds a new user-facing feature, reward, or content surface, confirm the PR body or linked-issue body names the discovery path - how a user finds this feature for the first time (nav link, onboarding step, tooltip, empty-state prompt, etc.). A new surface with no declared discovery path is a **Concern** ("discoverability gap"). A PR that explicitly defers the discovery path to a follow-up issue (with a link) is acceptable; an undocumented gap is not.
   - **Invariant-test continuity**: when a refactor moves a non-obvious invariant from one module to another - typical case: a hook's responsibility extracted into a Context provider, a helper's internals lifted to a parent - verify that the regression test for that invariant moved with the code (not silently deleted). The cookie-vs-settings separation in `useLocalePokemonName` is the worked example: the hook's test asserted "the hook reads from settings, NOT from the locale cookie" - that read moved into `PokemonLocaleContext` during the #1329 refactor, and the test was deleted without a replacement landing in the Provider tests. The reviewer caught it because the file header and AGENTS.md called the separation out as load-bearing. **Heuristic**: if the file header, an AGENTS.md section, or a doc topic file documents a non-obvious invariant for the changed module, search the test diff for the corresponding assertion. If it is absent from both the old and new test files, raise as a **Concern** - even when no functional bug appears, the next person to touch the code has no executable documentation of the constraint.
