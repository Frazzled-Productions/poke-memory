...
1. **Spawn one Agent per issue in the batch, in parallel** (single message, multiple `Agent` tool calls). Use the appropriate specialist subagent type per AGENTS.md "File ownership". Each agent works in an isolated worktree (`EnterWorktree`) so parallel branches don't collide.
   ...
   **Right-size each agent's task to avoid stalls (2026-06-07 retro).** A single agent handed a wide feature - one that touches many surfaces AND owes a full locale x state test matrix AND an e2e (e.g. a UI feature rendered on Pasture + Pokédex-detail + Journey + a new scope preset + i18n + onboarding) - reliably stalls part-way, leaving correct-but-incomplete uncommitted work. In the 2026-06-07 drain the #1766/#1767 mastery UI stalled THREE separate agents this way and had to be finished by orchestrator takeover. Prefer to **decompose a wide feature into separate agents / PRs** before dispatch: split the data/helper layer from the UI layer (land the pure helper + types first, as #1765/#1777 did for `legStatus.ts`, then build the UI on top), and/or split by surface. If a feature genuinely cannot be split, brief its agent to **commit + push after each sub-part** (helper, then each surface, then tests) so a stall strands nothing and takeover resumes from the last push. A tightly-scoped agent finishes; a mega-agent stalls.
...
2. **Each agent's prompt must include:**
   ...
   - "Add a `changelog.d/unreleased/<issue-number>-<slug>.md` fragment unless the change is internal-only (agent roster, workflow tweaks not visible to users). The frontmatter `kind` MUST be one of the EXACT values `added` / `changed` / `removed` / `deprecated` / `fixed` / `security` (NOT `fix`, NOT `change`), and the body must be `- ` bullet line(s). A wrong `kind` passes `eslint`/`typecheck`/`build`/`test` but fails CI's `lint:changelog` (the local `npm run lint` chain catches it only if the FULL chain is run) - 2026-06-11 #1846 shipped `kind: fix` and went red on CI."
   - "After ANY change to `messages/en.json` - adding, renaming, OR editing the text of an existing key - do both before pushing: (1) propagate the same change to `messages/ja.json`, `messages/zh-Hans.json`, `messages/zh-Hant.json` so the `lint:i18n` key-parity gate stays green, and (2) run `npm run generate:pseudo-locale` and commit the regenerated `messages/xx-pseudo.json` so the `lint:pseudo-locale` gate stays green. Editing an existing string's *value* (not just keys) still requires the pseudo-locale regen. The 2026-06-07 mastery batch tripped `lint:pseudo-locale` three times for missing this regen - each was caught by local lint but cost a gate cycle." (AGENTS.md "Message-catalogue completeness gate"; memory: `feedback_regenerate_pseudo_locale_on_catalogue_change`.)
...
0. **Agent-done != CI-green != merge-ready.** A sub-agent completing only means *the agent finished its work* - it does **not** mean the PR is green or mergeable. The agent's own local build-gate output is also **not** authoritative for the PR's required checks (the agent ran locally; CI runs `test` + `e2e` on the PR, plus `integration`/`migration-check` where applicable). Before reporting a PR as ready or merging it, query the PR's actual required-check status (`gh pr checks <PR> --json name,state`, or the MCP equivalent `pull_request_read` `get_status`/`get_check_runs`) and classify it explicitly as **ready / pending / failing** - never infer merge-readiness from an agent's self-report ("CI is running", "all checks pass") or its local gate. In status updates, distinguish "agent finished" from "CI green" from "merged", and make any headline count ("N of M done") reflect **merge-readiness**, not agent completion. (This codifies `feedback_verify_agent_reports` in-repo; the 2026-05-30 drain reported "4 of 5 done" while one PR had a failing migration-check and two had pending e2e, and the maintainer had to correct it - #1436.)
   ...
   **Read required checks with `--required`, not the raw rollup.** Use `gh pr checks <PR> --required` (deduped, latest run per check) to judge ready/pending/failing - do NOT count non-SUCCESS entries in `gh pr view --json statusCheckRollup`. When a PR branch is re-pushed (a changelog/coverage fix), the previous run's jobs are CANCELLED and a fresh run starts; the rollup keeps BOTH, so the cancelled duplicates read as failures and a naive count reports a green PR as red. When you do see CANCELLED entries, confirm against `gh run list --commit <headSha>` that the LATEST run per workflow succeeded before calling it a failure. (2026-06-11: #1873 - and briefly #1872/#1875 - were reported red while their `--required` checks were all green; the maintainer caught it. memory: `feedback_judge_ci_by_required_not_rollup`.)
