...
2. **Disable Auto Review.** `auto-review.yml` runs on PRs into `qa` as well as `main`. During a batch drain the in-session `code-reviewer` (Implementation step 3) is the review gate, so disable the workflow to avoid a redundant second review on every batch PR:
   ...
   **The harness permission classifier may BLOCK this command** as a shared-state mutation (it treats `gh workflow disable` like other GitHub-side mutations needing confirmation). If it is blocked, do **NOT** stop the run: proceed with Auto Review left **ENABLED**. With it on, `auto-review.yml` becomes the per-PR review gate (alongside the in-session `code-reviewer` pass), which is harmless — just a second review per PR. This is the observed-and-fine path. Only if the command actually *runs and errors* (e.g. genuine permissions failure, not a classifier block) surface the error and stop, since that signals a half-applied setup.
