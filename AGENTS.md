...
- **Global floor.** `coverage.thresholds` in `vitest.config.ts` (Statements 81 / Branches 75 / Functions 77 / Lines 83) is a regression guard set to the nearest whole percentage at or below the measured baseline. `vitest run --coverage` exits non-zero if overall coverage drops below the floor. Ratchet the floor *upward* as coverage improves — never lower it to make a red build pass. The `/batch-issues` skill ratchets these at the end of every session that touches product code (see its "End-of-session" steps); manual ratchets are also fine. Keep this paragraph in sync with `vitest.config.ts` so the documented values do not drift from the enforced ones.
