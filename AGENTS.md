...
**Branch protection on `main`.** A repository ruleset (`main-protection`, ID `16176438`) enforces the following on `main`: required status check `test` (the job from `ci.yml`) with strict-up-to-date policy, no force pushes, and linear history. The auto-PR App (BOT_APP_ID) is the sole bypass actor (`bypass_mode: always`) so bot-driven merges keep working. No human bypass — even repo admins go through the check. Manage via `gh api /repos/fraserbrookhouse/poke-memory/rulesets/16176438`; toggle `enforcement` between `active` / `evaluate` / `disabled` to stage changes.

**Pre-PR build gate.** `auto-issue.yml`'s implement job runs `npm ci` and instructs the orchestrator to run `npm run typecheck && npm run build && npm test` after pushing the branch but before opening the PR. If any step fails, the orchestrator is allowed up to two targeted fix attempts (commit + push + retry). After the second failure, it posts a comment with the last 80 lines of build output and stops without opening a PR — the branch stays pushed for manual inspection. The goal: catch type/build/test errors in the same run that produced them, instead of finding out via Vercel after the PR is open.

**CI check.** `ci.yml` runs `npm run typecheck && npm run build && npm test` on every `pull_request` event and on every push to `main`. The check is named `test` (the job ID; `CI` is the workflow name but only `test` is the API context used for required-check matching). Concurrent runs on the same ref are cancelled — only the latest push on a branch runs to completion. This is the per-commit gate; the pre-PR build gate inside `auto-issue.yml` is a separate earlier signal aimed at catching errors before the PR opens.
...
**Scope warning + `/split`.** When the planner posts its implementation plan, it first assesses scope against four thresholds (≥4 files, ≥3 surfaces, infrastructure + logic with ≥3 files, ≥6 acceptance criteria). When any threshold is crossed it appends a warning block. **Before proposing a split, the planner runs a coupling check** — it sketches the boundary it would draw between children and checks whether they would share surface area (same-named symbol whether existing or freshly introduced, same `localStorage` key or persistence target, same leaf module directory, or same file). If coupling is found — or if the planner cannot confidently rule it out — the warning still fires but `/split` is not offered: the recommendation is to proceed as a single issue (or to post a comment proposing a different breakdown that avoids the shared surface), since coupled children produce PRs that don't compose at merge time. When children are cleanly independent, the warning includes a numbered **Suggested split** block and offers `/split` as an alternative to `/go`. Commenting `/split` triggers the `split` job, which parses the proposed split titles from the most recent planner comment, creates a child GitHub issue per title (inheriting the parent's `priority:*` label plus an `auto` label), and links each child as a native sub-issue of the parent. The parent issue stays open as a tracker. **Each child inherits the `auto` label and so triggers its own `plan` run on creation** — this is intentional: the cascade lets the planner break the work down further per child without manual intervention. The `<!-- auto-split:N -->` marker comment is posted *before* the create-and-link loop runs, so a partial failure mid-loop still leaves an idempotency anchor — a re-run sees the marker and bails rather than duplicating issues. The `split` job itself exits non-zero when any link step fails, so the workflow shows red and the partial state is visible in the marker comment.
