...
- **Pre-PR build gate (exact command).** After pushing, run `npm run lint && npm run typecheck && npm run build && npm test && npm run test:coverage`. `npm run lint` is part of the gate: lint errors (e.g. a `no-restricted-syntax` ban) do **not** surface in typecheck/build/test, so omitting it ships a red PR (#1541). **Then run `npm run test:diff-coverage` - MANDATORY, not optional (#1649).** `npm run test:coverage` enforces only the global floor; the 90% per-diff patch bar runs in CI's `Test coverage report` and bit #1642 / #1646 (green locally, red on CI diff coverage). The `test:diff-coverage` script pipes `git diff origin/qa...HEAD` into `scripts/diff-coverage.mjs` against the freshly-produced `coverage/coverage-final.json`, so run it immediately after `test:coverage` while that file is present. Override the base ref for a main-targeting PR with `DIFF_COVERAGE_BASE=origin/main npm run test:diff-coverage`.
