...
The functional Playwright projects are `chromium`, `mobile-safari`, `desktop-webkit`, and `mobile-chrome` (see `playwright.config.ts`). `ci.yml`'s `e2e-browser` matrix runs `chromium` + `mobile-safari`; `desktop-webkit` and `mobile-chrome` widen local and dispatch coverage for Safari-desktop and Chrome-mobile quirks. All four functional projects ignore `e2e/visual.spec.ts` — the visual snapshot spec runs only under the `visual-chromium` / `visual-webkit` projects driven by `visual-regression.yml` below.
