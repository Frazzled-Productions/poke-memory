...
| **Post-push CI poll** | After pushing a fix commit, the agent polls `GET /repos/{owner}/{repo}/commits/{sha}/check-runs` (filtering for the `test` job) for up to 10 minutes (matching `ci.yml`'s `timeout-minutes: 10`). `<!-- auto-review:N+1 -->` is only posted when the check completes with `conclusion == "success"`. If CI fails or times out, the agent posts an explanatory comment and stops — no LGTM auto-review is posted on a red or unverified commit. |
