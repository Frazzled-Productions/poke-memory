...
| **Required secrets** | Prod: `SUPABASE_ACCESS_TOKEN` (Supabase PAT with read access), `SUPABASE_PROJECT_REF` (dashboard slug, e.g. `nvxvvtvnthsgdxgksmju`). QA: `QA_SUPABASE_ACCESS_TOKEN`, `QA_SUPABASE_PROJECT_REF`. The relevant pair must be set as repo secrets before the matching trigger can run; without them the script exits 2 with a clear error (the loud failure mode during the secrets-provisioning window). |
