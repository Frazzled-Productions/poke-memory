...
| **Why a dedicated workflow** | Snapshot baselines are platform-sensitive. macOS Core Text and Linux font anti-aliasing differ visibly (the reason README screenshots are macOS-only - see AGENTS.md → "Screenshots"). The job runs inside the pinned `mcr.microsoft.com/playwright` Docker image (tag in the workflow's `container.image`) so baselines are generated AND compared in the same deterministic Linux environment. The image tag MUST track the `@playwright/test` version in `package-lock.json`: `scripts/check-playwright-version-drift.mjs` enforces that for every workflow, plus the scripts and docs that carry a runnable `docker run` (#2074). Its test runs in every full vitest run, so on a PR drift fails the required `Test coverage report` check (`coverage.yml` runs the whole suite), and also `test` whenever `package.json` changes (vitest's `--changed` mode reruns everything then). A Dependabot Playwright bump therefore arrives red until someone moves every tag the failure lists; confirm the new image is published and ships the `.nvmrc` Node major first. |
| **Required check** | No - non-blocking, and a non-matching PR does not run it at all. |
| **Concurrency** | Per-ref, `cancel-in-progress: true`. |
| **Updating baselines** | When a UI change intentionally alters a surface, regenerate baselines inside the Docker image - never from macOS. Two paths: (1) **In CI** - dispatch `visual-baseline-update.yml` against the feature branch (owner-gated, refuses `main`/`qa`); it runs the same pinned image, commits the regenerated PNGs as `chore(visual): regenerate baselines`, and pushes to the dispatching branch. (2) **Locally**, from the repo root: `docker run --rm -v "$(pwd)":/work -w /work mcr.microsoft.com/playwright:v1.60.0-noble bash -c 'npm ci && npm run build && (npm start &) && npx wait-on http://localhost:3000 && npm run test:visual -- --update-snapshots'` - then commit the changed PNGs under `e2e/__screenshots__/`. |
...
| **What it does** | Runs inside the pinned `mcr.microsoft.com/playwright` image - the same pin `visual-regression.yml` uses for comparison. Checks out the dispatching branch via a `poke-memory-bot` App installation token, builds the app, serves it, runs `npm run test:visual -- --update-snapshots` to rewrite `e2e/__screenshots__/{visual-chromium,visual-webkit}/*.png`, then commits `chore(visual): regenerate baselines` and pushes to the dispatching branch. If the regeneration produces no diff, the job exits cleanly without an empty commit. |
| **Why an App token** | The push must be able to bypass branch protection on whatever feature-flow branch the dispatcher names (`GITHUB_TOKEN` cannot). The commit deliberately carries no `[skip ci]` marker: on a PR head that marker skips every check, required ones included (#1145). Instead `visual-regression.yml` ignores baseline-only pushes via `paths-ignore`, and `ci.yml` re-runs on purpose. |
