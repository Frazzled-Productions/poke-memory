...
// ---------------------------------------------------------------------------
// Higher-or-Lower entry flow (#1882)
// ---------------------------------------------------------------------------

describe("Higher-or-Lower launch-from-summary redesign (#1882)", () => {
  // Shared graduated state used to build "already-seen, not-due" name cards.
  // dueDate is far future so no card is queued; firstSeen ensures getSeenPokemon
  // counts the species.
  const seenGraduatedState = {
    stability: 25,
    difficulty: 5,
    elapsedDays: 25,
    scheduledDays: 25,
  ...
  // Graduated reverse card for Ivysaur (id 2). Prevents hydrateSession from
  // queueing a fresh unseen reverse card for that species (which would trigger
  // a newWall with maxNewReversePerDay=0).
  const graduatedReverseIvysaur = {
    ...FIXTURE_CARD,
    cardType: "reverse" as const,
    id: 2_000_002,   // REVERSE_ID_OFFSET + speciesId 2
    pokemonId: 2,
    subjectKey: "2",
    state: {
      ...seenGraduatedState,
  ...
  // Shared settings: no onboarding flags that would suppress anything, first
  // visit done so nudges are not blocked, all card-type extras off so only the
  // preloaded cards are in play, reverse cap=0 (extra reverse cards are loaded
  // directly in the session fixture, not generated by hydrateSession).
  const baseSettings = {
    masteryRepetitions: 3,
    maxNewPerDay: 10,
    maxReviewsPerDay: 100,
  ...
  it("shows enabled entry button and full game launch / back flow when seenPokemon.length >= 2", async () => {
    // Two seen species (Bulbasaur + Ivysaur): both have firstSeen set and are
    // included in the seed, so getSeenPokemon returns length 2.
    const ivysaur = FIXTURE_CARDS_4[1]; // id=2, name="Ivysaur"
    mockSeedPokemon.mockReturnValue([FIXTURE_CARD, ivysaur]);
    ...
    vi.mocked(loadSession).mockResolvedValueOnce({
      cards: [
        ...
        // Graduated reverse cards prevent hydrateSession adding new reverse work.
        GRADUATED_REVERSE_CARD,      // Bulbasaur reverse
        graduatedReverseIvysaur,     // Ivysaur reverse
      ],
      limits: DEFAULT_LIMITS,
    });
    ...
    // Summary view should render immediately (no due cards).
    await waitFor(() =>
      expect(screen.getByText(/all caught up/i)).toBeInTheDocument(),
    );

    // Entry button is visible and enabled.
    ...
    // Click the entry button -> game view.
    await user.click(entryBtn);

    // Game region is now visible.
    await waitFor(() =>
      expect(
    ...
    // Stat-guessing prompt is visible.
    expect(screen.getByText(/which has higher/i)).toBeInTheDocument();

    // Back control is visible.
    const backBtn = screen.getByRole("button", { name: /back to session summary/i });
    expect(backBtn).toBeInTheDocument();

    // Click back -> summary view restored.
    ...
    // Entry button is visible again (summary re-rendered).
    expect(
      screen.getByRole("button", { name: /play higher or lower/i }),
    ).toBeInTheDocument();
  });

  it("shows disabled entry button and helper text when seenPokemon.length === 1", async () => {
    // One seen species (Bulbasaur only). getSeenPokemon returns length 1 ->
    // disabled button + helper text should appear.
    mockSeedPokemon.mockReturnValue([FIXTURE_CARD]);
    mockLoadSettings.mockReturnValue(baseSettings);
    ...
    // Entry button is present but disabled.
    const entryBtn = screen.getByRole("button", { name: /not enough pokémon to play higher or lower/i });
    expect(entryBtn).toBeInTheDocument();
    expect(entryBtn).toBeDisabled();

    // Helper text is visible.
    expect(
      screen.getByText(/keep practising to unlock the stat guessing game/i),
    ).toBeInTheDocument();

    // Game region must not be present.
    expect(
      screen.queryByRole("region", { name: /higher or lower mini-game/i }),
    ).not.toBeInTheDocument();
  });

  it("hides the entry button entirely when seenPokemon.length === 0 (empty seed)", async () => {
    // Empty seed -> no cards, no seen Pokémon, session-complete screen appears
    // immediately. The entry area must not render at all.
    mockSeedPokemon.mockReturnValue([]);
    mockLoadSettings.mockReturnValue(baseSettings);
    ...
    // No entry button.
    expect(
      screen.queryByRole("button", { name: /play higher or lower/i }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /not enough pokémon/i }),
    ).not.toBeInTheDocument();
